...
data "aws_iam_policy_document" "eks_key" {
  ...
  statement {
    ...
    principals {
      type = "AWS"
      identifiers = [
        "arn:aws:iam::${local.context.aws_caller_identity_account_id}:root"
      ]
    }
  ...
  statement {
    ...
    principals {
      type = "AWS"
      identifiers = [
        "arn:aws:iam::${local.context.aws_caller_identity_account_id}:role/${local.cluster_iam_role_name}",
