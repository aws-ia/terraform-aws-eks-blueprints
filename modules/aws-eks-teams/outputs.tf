...
output "application_teams_iam_role_arn" {
  ...
  value = tomap({
    for k, v in aws_iam_role.application_team : k => v.arn
  })
}
...
output "application_teams_configure_kubectl" {
  ...
  value = tomap({
    for k, v in aws_iam_role.application_team : k => "aws eks --region ${data.aws_region.current.id} update-kubeconfig --name ${data.aws_eks_cluster.eks_cluster.name}  --role-arn ${v.arn}"
  })
}
