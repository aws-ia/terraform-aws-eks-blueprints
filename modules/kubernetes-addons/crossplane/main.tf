...
module "aws_provider_irsa" {
  ...
  irsa_iam_policies                 = concat(try([aws_iam_policy.aws_provider[0].arn], []), var.aws_provider.additional_irsa_policies)
...
module "terraform_provider_irsa" {
  ...
  irsa_iam_policies                 = concat(try([aws_iam_policy.terraform_provider[0].arn], []), var.terraform_provider.additional_irsa_policies)
  irsa_iam_role_path                = var.addon_context.irsa_iam_role_path
  irsa_iam_permissions_boundary     = var.addon_context.irsa_iam_permissions_boundary
  eks_cluster_id                    = var.addon_context.eks_cluster_id
  eks_oidc_provider_arn             = var.addon_context.eks_oidc_provider_arn
