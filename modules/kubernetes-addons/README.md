...
| <a name="input_coredns_helm_config"></a> [coredns\_helm\_config](#input\_coredns\_helm\_config) | CoreDNS Helm chart config | `any` | `{}` | no |
| <a name="input_coredns_use_managed_addon"></a> [coredns\_use\_managed\_addon](#input\_coredns\_use\_managed\_addon) | Determines whether to use the EKS mangaed addon (default, `true`) or to provision as self-managed/custom addon using Helm chart (`false`) | `bool` | `true` | no |
