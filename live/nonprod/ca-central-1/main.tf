...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw_routes" {
  ...
  tags               = { Name = "${local.cluster_name}-tgw" }

}

# resource "aws_route" "private_tgw_routes" {
# #  count = length(module.vpc-primary.private_route_table_ids) > 0 ? length(module.vpc-primary.private_route_table_ids) : 0

#   route_table_id         = "tgw-rtb-08336ddeb4fe0adfe"
...
module "aws-eks-accelerator-for-terraform" {
  # this will get the "defaults" from variables.tf in the root
  #live/nonpord/ca-centra-1
  source = "../../.."
  # this gets the platform_teams and application_teams from locals.tf file
