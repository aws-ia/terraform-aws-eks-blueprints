...
then
  ...
  echo "USAGE: ./5-spark-job-with-AMP-AMG.sh '<ENTER_EMR_VIRTUAL_CLUSTER_ID>' '<s3://ENTER_BUCKET_NAME>' '<EMR_JOB_EXECUTION_ROLE_ARN>'"
  exit 1
elif [ $# -gt 3 ];
then
  echo "$0: Too many arguments: $@"
  echo "Usage example-> ./5-spark-job-with-AMP-AMG.sh '<ENTER_EMR_VIRTUAL_CLUSTER_ID>' '<s3://ENTER_BUCKET_NAME>' '<EMR_JOB_EXECUTION_ROLE_ARN>'"
  exit 1
else
      ...
      "applicationConfiguration": [
          {
            ...
            "properties": {
              "spark.kubernetes.driver.podTemplateFile":"'"$SPARK_JOB_S3_PATH"'/pod-templates/spark-driver-fsx.yaml",
              "spark.kubernetes.executor.podTemplateFile":"'"$SPARK_JOB_S3_PATH"'/pod-templates/spark-executor-fsx.yaml",
              "spark.driver.memory":"2G",
              "spark.kubernetes.executor.podNamePrefix":"fsx-taxi-executor",
              "spark.local.dir":"/var/spark/spill/",
              "spark.kubernetes.driver.volumes.persistentVolumeClaim.sparkdata.options.claimName":"emr-eks-fsx-dynamic-claim",
              ...
              "spark.kubernetes.executor.volumes.persistentVolumeClaim.spark-local-dir-spill.options.claimName":"emr-eks-fsx-dynamic-claim",
