...
then
  ...
  echo "USAGE: ./fsx-static-spark.sh '<ENTER_EMR_VIRTUAL_CLUSTER_ID>' '<s3://ENTER_BUCKET_NAME>' '<EMR_JOB_EXECUTION_ROLE_ARN>'"
  exit 1
elif [ $# -gt 3 ];
then
  echo "$0: Too many arguments: $@"
  echo "Usage example-> ./fsx-static-spark.sh '<ENTER_EMR_VIRTUAL_CLUSTER_ID>' '<s3://ENTER_BUCKET_NAME>' '<EMR_JOB_EXECUTION_ROLE_ARN>'"
  exit 1
else
    ...
    --job-driver '{
      "sparkSubmitJobDriver": {
        ...
        "entryPointArguments": ["'"$SPARK_JOB_S3_PATH"'/input/taxi-trip-data/",
          "'"$SPARK_JOB_S3_PATH"'/output/static/taxi-trip-data/"
        ],
      ...
      "applicationConfiguration": [
          {
            ...
            "properties": {
              "spark.kubernetes.driver.podTemplateFile":"'"$SPARK_JOB_S3_PATH"'/pod-templates/fsx-static-spark-driver.yaml",
              "spark.kubernetes.executor.podTemplateFile":"'"$SPARK_JOB_S3_PATH"'/pod-templates/fsx-static-spark-executor.yaml",
              "spark.driver.memory":"2G",
              "spark.kubernetes.executor.podNamePrefix":"ny-taxi-exec",
