...
then
  ...
  echo "Usage example-> ./fsx-dynamic-spark.sh '<ENTER_EMR_VIRTUAL_CLUSTER_ID>' '<s3://ENTER_BUCKET_NAME>' '<EMR_JOB_EXECUTION_ROLE_ARN>'"
  exit 1
else
    ...
    --job-driver '{
      "sparkSubmitJobDriver": {
        ...
        "entryPointArguments": ["'"$SPARK_JOB_S3_PATH"'/input/taxi-trip-data/",
          "'"$SPARK_JOB_S3_PATH"'/output/dynamic/taxi-trip-data/"
        ],
