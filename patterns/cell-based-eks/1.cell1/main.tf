...
module "eks" {
  ...
  fargate_profiles = {
    karpenter = {
      ...
      subnet_ids = [data.terraform_remote_state.vpc.outputs.subnet_ids[0]]
    }
    kube_system = {
      name = "kube-system"
      selectors = [
...
resource "kubectl_manifest" "karpenter_node_template" {
  yaml_body = <<-YAML
    ...
    spec:
      subnetSelector:
        aws-ids: ${data.terraform_remote_state.vpc.outputs.subnet_ids[0]}
